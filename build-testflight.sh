...
echo "=== 0. Configure depends for tvOS (sdk 26.4) ==="
# Makefile.include is generated by tools/depends/configure and records the
# platform + source path. Re-run it so building tvOS after iOS (or after a
# repo move) always uses the correct toolchain/prefix.
( cd "$KODI_SRC/tools/depends" && ./configure --with-platform=tvos --with-sdk=26.4 >/dev/null )
